#!/usr/bin/env bash
# Stale-position payload sweep: all variants run --scramble + warmup=0 +
# promotion disabled, so pages are at random frame positions and predictions
# are wrong. Sweeps payload_bytes ∈ {0, 256, 1024, 4096, 16384}.
set -euo pipefail
...
# Disable PT promotion so pages stay at scrambled positions during the run.
export PT_PROMOTE_PROB_NO_DEMOTE=4294967295
export PT_PROMOTE_PROB_DEMOTE=4294967295

TARGET="./target/release"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
OUTDIR="$SCRIPT_DIR/results/payload_sweep_stale_${TIMESTAMP}"
mkdir -p "$OUTDIR"
SUMMARY="$OUTDIR/summary.txt"

declare -a VARIANTS=(
    "bp_clock|LIPAH-stale|--scramble"
    "bp_predicache|PrediCache|--scramble"
    "bp_pt_bucket_v2_congee|EnhancedPrediCache|--scramble"
    "bp_lapt|EnhancedPrediCache-OP|--scramble"
)

declare -a PAYLOADS=(0 256 1024 4096 16384)

echo "=== Payload sweep, sequential scan, STALE (scrambled, no warmup, no promotion) ===" | tee "$SUMMARY"
echo "Config: n=$N f=$F t=$T containers=$C warmup=${W}s exec=${S}s" | tee -a "$SUMMARY"
echo "PT_PROMOTE_PROB_NO_DEMOTE=$PT_PROMOTE_PROB_NO_DEMOTE" | tee -a "$SUMMARY"
echo "Date: $(date)" | tee -a "$SUMMARY"
echo ""  | tee -a "$SUMMARY"

# Build all variants up-front (one binary per feature set).
