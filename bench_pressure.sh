#!/usr/bin/env bash
# Memory pressure benchmark: 10w10t with 1GB BP (working set ~3GB).
# Tests promotion/demotion effects under eviction.
set -euo pipefail

W=10
T=10
BP=1  # 1 GB buffer pool (vs ~3 GB working set)
WARMUP=3
EXEC=15
BIN_SRC="tpcc_profile_neworder"

TARGET="./target/release"
OUTDIR="bench_pressure_results"
mkdir -p "$OUTDIR"
...
echo "=== Memory Pressure Benchmark ===" | tee "$SUMMARY"
...
# Header
printf "%-12s %-10s %-10s %-8s %-8s %-8s %-8s %-8s %-8s %-8s\n" \
