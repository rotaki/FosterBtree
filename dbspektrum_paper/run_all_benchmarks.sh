...
#   T=44  S=15  W_SAT=10  TRIALS=3
#   NUM_KEYS=2000000  KEY_SIZE=10  VAL_MIN=100 VAL_MAX=100  SCAN_SIZE=1000  EXEC_SECS=15
...
# Variants: features|label. No `pt_counts` / `pt_profile` — counter atomics
# would slow the hot path and we don't need them for throughput plots.
#
# The four PrediCache cells encode the 2×2 ablation (placement × bypass)
# via `pc_prefix_hash` / `pc_bypass` cfg flags inside `predicache.rs`:
#   PrediCache      = (uniform, always-probe) — vanilla
#   prefix          = (prefix,  always-probe)
#   bypass          = (uniform, bypass)
#   both            = (prefix,  bypass)
declare -a VARIANTS=(
    "bp_clock|LIPAH"
    "bp_predicache|PrediCache"
    "bp_predicache,pc_prefix_hash|prefix"
    "bp_predicache,pc_bypass|bypass"
    "bp_predicache,pc_prefix_hash,pc_bypass|both"
)
    ...
    for entry in "${VARIANTS[@]}"; do
        ...
        for p in "${PAYLOADS[@]}"; do
            for trial in $(seq 1 "$TRIALS"); do
                ...
                # Always exercise the callback `read_page_with` path so the API
                # is symmetric across variants — isolates placement/lookup
                # differences from BP-API differences.
