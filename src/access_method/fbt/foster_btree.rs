...
fn is_parent_and_child(parent: &Page, child: &Page) -> bool {
    ...
    deserialize_page_id(parent.get_val(slot_id)) == Some(child.page_id())
}
...
fn is_foster_relationship(parent: &Page, child: &Page) -> bool {
    parent.level() == child.level()
        ...
        && deserialize_page_id(parent.get_foster_val()) == Some(child.page_id())
}
...
fn descend_root(root: &mut FrameWriteGuard, child: &mut FrameWriteGuard) {
    ...
    assert_eq!(deserialize_page_id(root.get_val(1)), Some(child.page_id()));
...
fn ascend_root(root: &mut FrameWriteGuard, child: &mut FrameWriteGuard) {
    ...
    let child_id = InnerVal::new_with_frame_id(child.page_id(), child.frame_id());
    ...
    fn modify_structure(
        ...
        match op {
            OpType::Merge => {
                ...
                merge(this, child);
                self.unused_pages.push(child.page_id()).unwrap();
            }
            OpType::LoadBalance => {
...
impl<T: MemPool> FosterBtreeCursor<T> {
    ...
    fn initialize(&mut self) {
        ...
        loop {
            ...
            if this_page.is_leaf() {
                ...
                // If query is 4 -> Go to the foster child
                // If query is 5 -> Go to the foster child
                // If query is 6 -> Go to the foster child
                // If query is 7 -> Go to the foster child
                ...
                let is_high_fence = leaf_page.high_fence_slot_id() == slot;
                // The slot should not be a foster child slot. Otherwise it should have been traversed.
                debug_assert!(
                    !leaf_page.has_foster_child() || leaf_page.foster_child_slot_id() != slot
                );
                ...
                if is_high_fence {
                    return self.go_to_next_kv();
                } else {
                    // We read the key-value pair in this page at the current slot
                    return;
                }
            }
            ...
            current_page = next_page;
            self.visited.push(page_key); // Push the visiting page to the stack
        }
    }
    ...
    // If there is no more leaf pages in the range, the current_leaf_page is set to None.
    pub fn go_to_next_kv(&mut self) {
        if self.finished {
            return;
        }
...
pub struct FosterBtreeAppendOnlyCursor<T: MemPool> {
    cursor: FosterBtreeCursor<T>,
}
...
impl<T: MemPool> FosterBtreeAppendOnlyCursor<T> {
    ...
    pub fn get_kv(&self) -> Option<NonUniqueKeyValueType> {
        ...
        Some(((key.to_vec(), suffix), val))
    }

    pub fn opportunistic_update(&mut self, val: &[u8], make_dirty: bool) {
        self.cursor.opportunistic_update(val, make_dirty);
    }
...
impl<T: MemPool> FosterBtreeAppendOnly<T> {
    ...
    pub fn traverse_to_leaf_for_read_with_hint(
        ...
        self.fbt
            .traverse_to_leaf_for_read_with_hint(&suffixed_key, hint)
    }
    ...
    pub fn traverse_to_leaf_for_write_with_hint(
        ...
        suffixed_key.extend(0_u32.to_be_bytes());
        self.fbt
            .traverse_to_leaf_for_write_with_hint(&suffixed_key, hint)
    }
...
mod tests {
    ...
    #[case::bp_clock(get_test_bp_clock(1))]
    ...
    fn test_page_merge_detail<T: MemPool>(
        ...
        let foster_val = InnerVal::new_with_frame_id(p1.page_id(), p1.frame_id());
    ...
    #[case::bp_clock(get_test_bp_clock(2))]
    #[case::in_mem(get_in_mem_pool())]
    #[case::vmc(get_test_vmcache::<true>(2))]
    ...
    fn test_page_adopt_detail<T: MemPool>(
        ...
        parent.set_level(1);
        let val = InnerVal::new_with_frame_id(child0.page_id(), child0.frame_id());
        ...
        let foster_val = InnerVal::new_with_frame_id(child1.page_id(), child1.frame_id());
    ...
    #[case::bp_clock(get_test_bp_clock(3))]
    #[case::in_mem(get_in_mem_pool())]
    ...
    fn test_page_anti_adopt_detail<T: MemPool>(
        ...
        let val = InnerVal::new_with_frame_id(child0.page_id(), child0.frame_id());
        parent.insert(&k0, &val.to_bytes(), false);
        let val = InnerVal::new_with_frame_id(child1.page_id(), child1.frame_id());
    ...
    fn test_root_page_ascend<T: MemPool>(#[case] bp: Arc<T>) {
        ...
        let val = InnerVal::new_with_frame_id(foster_child.page_id(), foster_child.frame_id());
    ...
    fn test_root_page_descend<T: MemPool>(#[case] bp: Arc<T>) {
        ...
        root.init_as_root();
        root.increment_level();
        let val = InnerVal::new_with_frame_id(child.page_id(), child.frame_id());
    ...
    fn build_foster_relationship<T: MemPool>(
        ...
        this.set_has_foster_child(true);
        let val = InnerVal::new_with_frame_id(foster.page_id(), foster.frame_id());
    ...
    #[case::bp_clock(get_test_bp_clock(5))]
    #[case::in_mem(get_in_mem_pool())]
    #[case::vmc(get_test_vmcache::<true>(5))]
    ...
    fn test_cursor<T: MemPool>(#[case] bp: Arc<T>) {
        ...
        let start_key = to_bytes(2);
        let end_key = to_bytes(7);
        let mut cursor = FosterBtreeCursor::new(&btree, &start_key, &end_key);
        let mut count = 0;
        while let Some((key, current_val)) = cursor.get_kv() {
            let key = from_bytes(&key);
            println!(
                "**************************** Scanning key {} **************************",
                key
    ...
    #[case::in_mem(get_in_mem_pool())]
    #[ignore]
    // #[case::bp(get_test_bp(1024))]
    // #[case::bp_clock(get_test_bp_clock(1024))]
    // #[case::vmc(get_test_vmcache::<false>(1024))]
    ...
    fn test_cursor_heavy<T: MemPool>(#[case] bp: Arc<T>) {
        ...
        let mut cursor = FosterBtreeCursor::new(&btree, &[], &[]);
        let mut count = 0;
        while let Some((key, _)) = cursor.get_kv() {
            let key = from_bytes(&key);
            println!(
                "**************************** Scanning key {} **************************",
                key
    ...
    #[case::bp_clock(get_test_bp_clock(100))]
    #[case::in_mem(get_in_mem_pool())]
    #[case::vmc(get_test_vmcache::<true>(100))]
    ...
    #[rstest]
    #[case::in_mem(get_in_mem_pool())]
    // #[case::bp_clock(get_test_bp_clock(512))]
    ...
    #[case::bp_clock(get_test_bp_clock(100))]
    #[case::in_mem(get_in_mem_pool())]
    // #[case::vmc(get_test_vmcache::<false>(100))]
