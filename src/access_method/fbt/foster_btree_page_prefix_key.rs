...
mod slot {
    ...
    pub struct Slot {
        ...
        prefix_size: u8,  // The size of the prefix
        suffix_size: u32, // The size of the suffix
    ...
    impl Slot {
        pub fn from_bytes(bytes: [u8; SLOT_SIZE]) -> Self {
            ...
            let prefix_size = bytes[current_pos];
            current_pos += std::mem::size_of::<u8>();
        ...
        pub fn to_bytes(&self) -> [u8; SLOT_SIZE] {
            ...
            let suffix_size_bytes = self.suffix_size.to_be_bytes();
            ...
            bytes[current_pos] = self.prefix_size;
            current_pos += std::mem::size_of::<u8>();

            bytes[current_pos..current_pos + std::mem::size_of::<u32>()]
                .copy_from_slice(&suffix_size_bytes);
            current_pos += std::mem::size_of::<u32>();
        ...
        pub fn prefix_size(&self) -> u8 {
            self.prefix_size
        }

        pub fn set_prefix_size(&mut self, prefix_size: u8) {
        ...
        pub fn set_suffix_size(&mut self, suffix_size: u32) {
        ...
        pub fn total_size(&self) -> u32 {
            SLOT_SIZE as u32 + KEY_PREFIX_SIZE as u32 + self.suffix_size + self.value_size
        }
    }
}
...
fn separate_prefix_suffix(key: &[u8]) -> (&[u8], &[u8]) {
    let size = KEY_PREFIX_SIZE.min(key.len());
    (&key[..size], &key[size..])
}
...
pub enum BTreeKey<'a> {
    MinusInfty,
    Normal(&'a [u8]),
    PSKey(&'a Page, u32), // Page and slot_id
    PlusInfty,
}
...
impl<'a> BTreeKey<'a> {
    ...
    pub fn to_vec(&self) -> Vec<u8> {
        match self {
            BTreeKey::MinusInfty => vec![],
            BTreeKey::Normal(key) => key.to_vec(),
            BTreeKey::PSKey(page, slot_id) => {
                let slot = page.slot(*slot_id).unwrap();
                let offset = slot.offset() as usize;
                let prefix_size = slot.prefix_size() as usize;
                let suffix_size = slot.suffix_size() as usize;
                let prefix_offset = page.slot_offset(*slot_id) + SLOT_SIZE;
                let mut key = Vec::with_capacity(prefix_size + suffix_size);
                key.extend_from_slice(&page[prefix_offset..prefix_offset + prefix_size]);
                key.extend_from_slice(&page[offset..offset + suffix_size]);
                key
            }
            BTreeKey::PlusInfty => vec![],
        }
    }
}
...
impl PartialEq for BTreeKey<'_> {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (BTreeKey::MinusInfty, _) => false,
            (_, BTreeKey::MinusInfty) => false,
            (BTreeKey::PlusInfty, BTreeKey::PlusInfty) => true,
            (BTreeKey::PlusInfty, _) => false,
            (_, BTreeKey::PlusInfty) => false,
            (BTreeKey::Normal(key1), BTreeKey::Normal(key2)) => key1 == key2,
            (BTreeKey::Normal(key), BTreeKey::PSKey(page, slot_id))
            | (BTreeKey::PSKey(page, slot_id), BTreeKey::Normal(key)) => {
                let slot = page.slot(*slot_id).unwrap();
                let offset = slot.offset() as usize;
                let prefix_size = slot.prefix_size() as usize;
                let suffix_size = slot.suffix_size() as usize;
                let prefix_offset = page.slot_offset(*slot_id) + SLOT_SIZE;
                if prefix_size > key.len() {
                    return false;
                }
                let (prefix1, suffix1) = separate_prefix_suffix(key);
                let prefix = &page[prefix_offset..prefix_offset + prefix_size];
                if prefix1 != prefix {
                    return false;
                }
                let suffix = &page[offset..offset + suffix_size];
                suffix1 == suffix
            }
            (BTreeKey::PSKey(page1, slot_id1), BTreeKey::PSKey(page2, slot_id2)) => {
                let slot1 = page1.slot(*slot_id1).unwrap();
                let offset1 = slot1.offset() as usize;
                let prefix_size1 = slot1.prefix_size() as usize;
                let suffix_size1 = slot1.suffix_size() as usize;
                let prefix_offset1 = page1.slot_offset(*slot_id1) + SLOT_SIZE;

                let slot2 = page2.slot(*slot_id2).unwrap();
                let offset2 = slot2.offset() as usize;
                let prefix_size2 = slot2.prefix_size() as usize;
                let suffix_size2 = slot2.suffix_size() as usize;
                let prefix_offset2 = page2.slot_offset(*slot_id2) + SLOT_SIZE;

                if prefix_size1 != prefix_size2 || suffix_size1 != suffix_size2 {
                    return false;
                }

                let prefix1 = &page1[prefix_offset1..prefix_offset1 + prefix_size1];
                let prefix2 = &page2[prefix_offset2..prefix_offset2 + prefix_size2];

                if prefix1 != prefix2 {
                    return false;
                }

                let suffix1 = &page1[offset1..offset1 + suffix_size1];
                let suffix2 = &page2[offset2..offset2 + suffix_size2];
...
impl PartialOrd for BTreeKey<'_> {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        match (self, other) {
            ...
            (BTreeKey::MinusInfty, _) => Some(std::cmp::Ordering::Less),
            (_, BTreeKey::MinusInfty) => Some(std::cmp::Ordering::Greater),
            (BTreeKey::PlusInfty, BTreeKey::PlusInfty) => Some(std::cmp::Ordering::Equal),
            (BTreeKey::PlusInfty, _) => Some(std::cmp::Ordering::Greater),
            (_, BTreeKey::PlusInfty) => Some(std::cmp::Ordering::Less),
            ...
            (BTreeKey::Normal(key), BTreeKey::PSKey(page, slot_id)) => {
                ...
                let prefix_offset = page.slot_offset(*slot_id) + SLOT_SIZE;
                let (prefix1, suffix1) = separate_prefix_suffix(key);
                match prefix1.cmp(&page[prefix_offset..prefix_offset + prefix_size]) {
                    std::cmp::Ordering::Equal => {
                        let offset = slot.offset() as usize;
                        let suffix_size = slot.suffix_size() as usize;
                        Some(suffix1.cmp(&page[offset..offset + suffix_size]))
                    }
                    ord => Some(ord),
                }
            }
            (BTreeKey::PSKey(page, slot_id), BTreeKey::Normal(key)) => {
                ...
                let prefix_size = slot.prefix_size() as usize;
                let prefix_offset = page.slot_offset(*slot_id) + SLOT_SIZE;
                let (prefix2, suffix2) = separate_prefix_suffix(key);
                match page[prefix_offset..prefix_offset + prefix_size].cmp(prefix2) {
                    std::cmp::Ordering::Equal => {
                        let offset = slot.offset() as usize;
                        let suffix_size = slot.suffix_size() as usize;
                        Some(page[offset..offset + suffix_size].cmp(suffix2))
                    }
                    ord => Some(ord),
                }
            }
...
pub trait FosterBtreePage {
    ...
    fn prefix(&self, slot_id: u32) -> Option<&[u8]>;
    fn append_slot(&mut self, slot: &Slot, key_prefix: &[u8]);
    fn update_slot(&mut self, slot_id: u32, slot: &Slot, key_prefix: Option<&[u8]>);
    ...
    fn empty(&self) -> bool;
    fn get_raw_key(&self, slot_id: u32) -> Vec<u8>;
    fn get_btree_key(&self, slot_id: u32) -> BTreeKey;
    fn get_foster_key(&self) -> Vec<u8>;
    fn get_foster_val(&self) -> &[u8];
    fn get_val(&self, slot_id: u32) -> &[u8];
    fn inside_range(&self, key: &BTreeKey) -> bool;
    fn lower_bound_slot_id(&self, key: &BTreeKey) -> u32;
    fn upper_bound_slot_id(&self, key: &BTreeKey) -> u32;
    fn find_slot_id(&self, key: &BTreeKey) -> Option<u32>;
    ...
    fn remove_at(&mut self, slot_id: u32);
...
impl FosterBtreePage for Page {
    ...
    fn bytes_used(&self, range: std::ops::Range<u32>) -> u32 {
        ...
        for i in range {
            ...
            sum_used += slot.suffix_size() as usize + slot.value_size() as usize;
            sum_used += SLOT_SIZE;
            sum_used += KEY_PREFIX_SIZE;
        }
        sum_used as u32
    }

    fn bytes_needed(&self, key: &[u8], value: &[u8]) -> u32 {
        let suffix_size = key.len() as i32 - KEY_PREFIX_SIZE as i32;
        ((0.max(suffix_size)) as usize + value.len() + SLOT_SIZE + KEY_PREFIX_SIZE) as u32
    }
    ...
    fn contiguous_free_space(&self) -> u32 {
        ...
        rec_start_offset - next_slot_offset as u32
    }
    ...
    fn slot_offset(&self, slot_id: u32) -> usize {
        PAGE_HEADER_SIZE + slot_id as usize * (SLOT_SIZE + KEY_PREFIX_SIZE)
    }
    ...
    fn prefix(&self, slot_id: u32) -> Option<&[u8]> {
        if slot_id < self.slot_count() {
            let prefix_size = self.slot(slot_id).unwrap().prefix_size() as usize;
            let offset = self.slot_offset(slot_id) + SLOT_SIZE; // This is the start offset of the prefix
            Some(&self[offset..offset + prefix_size])
        } else {
            None
        }
    }
    ...
    fn append_slot(&mut self, slot: &Slot, key_prefix: &[u8]) {
        ...
        let slot_offset = self.slot_offset(slot_id);
        self[slot_offset..slot_offset + SLOT_SIZE].copy_from_slice(&slot.to_bytes());
        self[slot_offset + SLOT_SIZE..slot_offset + SLOT_SIZE + key_prefix.len()]
            .copy_from_slice(key_prefix);
    ...
    fn get_low_fence(&self) -> BTreeKey {
        if self.is_left_most() {
            ...
            BTreeKey::PSKey(self, self.low_fence_slot_id())
        }
    }
    ...
    fn get_high_fence(&self) -> BTreeKey {
        if self.is_right_most() {
            ...
            BTreeKey::PSKey(self, self.high_fence_slot_id())
        }
    }
    ...
    fn compact_space(&mut self) {
        let rec_mem_usage = self.total_bytes_used()
            ...
            - (SLOT_SIZE + KEY_PREFIX_SIZE) as u32 * self.slot_count();
            ...
            std::cmp::Ordering::Less => {
                ...
                for i in 0..self.slot_count() {
                    if let Some(mut slot) = self.slot(i) {
                        ...
                        let size = suffix_size + value_size;
                        current_size += size;
    ...
    fn get_raw_key(&self, slot_id: u32) -> Vec<u8> {
        ...
        let prefix_size = slot.prefix_size() as usize;
        let suffix_size = slot.suffix_size() as usize;
        let prefix_offset = self.slot_offset(slot_id) + SLOT_SIZE;
        let prefix = &self[prefix_offset..prefix_offset + prefix_size];
        let suffix = &self[offset..offset + suffix_size];
        let mut key = Vec::with_capacity(prefix_size + suffix_size);
    ...
    fn get_btree_key(&self, slot_id: u32) -> BTreeKey {
        if slot_id == self.low_fence_slot_id() {
            ...
            let suffix_size = slot.suffix_size() as usize;
            if prefix_size == 0 {
                debug_assert_eq!(suffix_size, 0);
                BTreeKey::MinusInfty
            } else {
                BTreeKey::PSKey(self, slot_id)
            }
        }
    }
    ...
    fn get_val(&self, slot_id: u32) -> &[u8] {
        ...
        let suffix_size = slot.suffix_size() as usize;
        let value_size = slot.value_size() as usize;
        &self[offset + suffix_size..offset + suffix_size + value_size]
    }
    ...
    fn insert_at(&mut self, slot_id: u32, key: &[u8], value: &[u8], is_ghost: bool) -> bool {
        let (prefix, suffix) = separate_prefix_suffix(key);
        let rec_size = suffix.len() + value.len();
        if SLOT_SIZE + KEY_PREFIX_SIZE + rec_size > self.contiguous_free_space() as usize {
            if SLOT_SIZE + KEY_PREFIX_SIZE + rec_size > self.total_free_space() as usize {
                return false;
            } else {
                self.compact_space();
        ...
        match start.cmp(&end) {
            ...
            std::cmp::Ordering::Equal => {
                ...
                self[offset as usize..offset as usize + suffix.len()].copy_from_slice(suffix);
                self[offset as usize + suffix.len()..offset as usize + rec_size]
            ...
            std::cmp::Ordering::Less => {
                ...
                self.copy_within(start..end, start + SLOT_SIZE + KEY_PREFIX_SIZE);
                ...
                self.update_slot(slot_id, &slot, Some(prefix));
    ...
    fn update_at(&mut self, slot_id: u32, key: Option<&[u8]>, value: &[u8]) -> bool {
        assert!(slot_id < self.slot_count());
        let mut slot = self.slot(slot_id).unwrap();
        let old_rec_size = slot.suffix_size() as usize + slot.value_size() as usize;
        let new_rec_size = key
            .map(|k| separate_prefix_suffix(k).1.len())
            .unwrap_or(slot.suffix_size() as usize)
            + value.len();
        if new_rec_size <= old_rec_size {
            ...
            if let Some(key) = key {
                // Key changes
                let (prefix, suffix) = separate_prefix_suffix(key);
                self[offset..offset + suffix.len()].copy_from_slice(suffix);
                self[offset + suffix.len()..offset + new_rec_size].copy_from_slice(value);
                slot.set_prefix_size(prefix.len() as u8);
                slot.set_suffix_size(suffix.len() as u32);
                slot.set_value_size(value.len() as u32);
                self.update_slot(slot_id, &slot, Some(prefix));
            } else {
                // Key is the same
                self[offset + slot.suffix_size() as usize..offset + new_rec_size]
                    .copy_from_slice(value);
                slot.set_value_size(value.len() as u32);
                self.update_slot(slot_id, &slot, None);
            }
            self.set_total_bytes_used(
                self.total_bytes_used() - old_rec_size as u32 + new_rec_size as u32,
            );
            return true;
        }
        ...
        if let Some(key) = key {
            ...
            let (prefix, suffix) = separate_prefix_suffix(key);
            self[offset as usize..offset as usize + suffix.len()].copy_from_slice(suffix);
            self[offset as usize + suffix.len()..offset as usize + new_rec_size]
                .copy_from_slice(value);
            // Update the slot
            ...
            self.update_slot(slot_id, &new_slot, Some(prefix));
            self.set_total_bytes_used(
                self.total_bytes_used() - old_rec_size as u32 + new_rec_size as u32,
            );
        } else {
            // Key is the same
            self.copy_within(
                slot.offset() as usize..slot.offset() as usize + slot.suffix_size() as usize,
                offset as usize,
            );
            self[offset as usize + slot.suffix_size() as usize..offset as usize + new_rec_size]
                .copy_from_slice(value);
            // Update the slot
            let new_slot = Slot::new(
                is_ghost,
            ...
            self.update_slot(slot_id, &new_slot, None);
            self.set_total_bytes_used(
                self.total_bytes_used() - old_rec_size as u32 + new_rec_size as u32,
            );
        };
    ...
    fn insert(&mut self, key: &[u8], value: &[u8], is_ghost: bool) -> bool {
        ...
        // Insert at slot_id
        self.insert_at(slot_id, key, value, is_ghost)
    }
    ...
    fn append_sorted<K: AsRef<[u8]>, V: AsRef<[u8]>>(&mut self, recs: &[(bool, K, V)]) -> bool {
        ...
        #[cfg(any(test, debug_assertions))]
        {
            ...
            if self.low_fence_slot_id() == self.high_fence_slot_id() - 1 {
                debug_assert!(last_key <= BTreeKey::Normal(recs[0].1.as_ref()));
            } else {
                debug_assert!(last_key < BTreeKey::Normal(recs[0].1.as_ref()));
            }
            assert!(BTreeKey::Normal(recs[recs.len() - 1].1.as_ref()) < high_fence);
        ...
        let inserting_size = recs
            .iter()
            .map(|(_, k, v)| separate_prefix_suffix(k.as_ref()).1.len() + v.as_ref().len())
            .sum::<usize>()
            + recs.len() * (SLOT_SIZE + KEY_PREFIX_SIZE);
        ...
        let high_fence_prefix = self.prefix(self.high_fence_slot_id()).unwrap().to_vec();
            ...
            .map(|(is_ghost, k, v)| (is_ghost, k.as_ref(), v.as_ref()))
        {
            ...
            self[offset as usize..offset as usize + suffix.len()].copy_from_slice(suffix);
            self[offset as usize + suffix.len()..offset as usize + rec_size].copy_from_slice(value);
            self.append_slot(&slot, prefix);
        }

        self.append_slot(&high_fence_slot, &high_fence_prefix); // Restore the high fence slot
    ...
    fn remove_range(&mut self, from: u32, to: u32) {
        if from == to {
            return;
        }
        if from > to {
        ...
        let end = self.slot_offset(self.slot_count());
        let new_start = start - (to - from) as usize * (SLOT_SIZE + KEY_PREFIX_SIZE);
    ...
    fn check_rec_start_offset_match_slots(&self) {
        let mut rec_start_offset = u32::MAX;
        for i in 0..self.slot_count() {
            let slot = self.slot(i).unwrap();
    ...
    fn check_ideal_space_usage(&self) {
        ...
        for i in 0..self.slot_count() {
            if let Some(slot) = self.slot(i) {
                rec_mem_usage += slot.suffix_size() + slot.value_size();
            }
        }
        let ideal_start_offset = self.len() as u32 - rec_mem_usage;
    ...
    fn print_all<K: Fn(&[u8]) -> String>(&self, key_fn: K) {
        ...
        for i in 0..self.slot_count() {
            ...
            print!("{}({:?}, {:?})", sep, key_fn(&key.to_vec()), value,);
            sep = ", ";
        }
        println!("]");
    }
...
mod tests {
    ...
    */
    ...
    fn test_insert_out_of_range() {
        ...
        fbt_page.insert("a".as_bytes(), "aa".as_bytes(), false);
    }
    ...
    fn test_insert_duplicate() {
        ...
        fbt_page.insert("c".as_bytes(), "c".as_bytes(), false);
    ...
    fn test_insert_sorted() {
        ...
        let recs = vec![
            (false, "b".as_bytes(), "bb".as_bytes()),
            (false, "c".as_bytes(), "cc".as_bytes()),
        ];
    ...
    fn test_remove_range() {
        ...
        let recs = vec![
            (false, "b".as_bytes(), "c".as_bytes()),
            (false, "bb".as_bytes(), "cc".as_bytes()),
            (false, "bbb".as_bytes(), "ccc".as_bytes()),
            (false, "bbbb".as_bytes(), "cccc".as_bytes()),
            (false, "bbbbb".as_bytes(), "ccccc".as_bytes()),
            (false, "bbbbbb".as_bytes(), "cccccc".as_bytes()),
        ];
        assert!(fbt_page.append_sorted(&recs));
        fbt_page.run_consistency_checks(true);
    ...
    #[test]
    fn test_ghost_record() {
        // Turn some records into ghost record by setting the
        // ghost bit in the slot.
        let mut fbt_page = Page::new_empty();
        fbt_page.init();
        fbt_page.run_consistency_checks(true);
        ...
        fbt_page.insert("b".as_bytes(), "bb".as_bytes(), true);
        fbt_page.run_consistency_checks(true);
        fbt_page.insert("c".as_bytes(), "cc".as_bytes(), true);
        fbt_page.run_consistency_checks(true);
        fbt_page.compact_space();
        fbt_page.run_consistency_checks(true);
        assert_eq!(fbt_page.slot_count(), 4); // 2 records + 2 fence slots
        assert_eq!(fbt_page.lower_bound_slot_id(&BTreeKey::str("b")), 0);
        assert_eq!(fbt_page.upper_bound_slot_id(&BTreeKey::str("b")), 2);
        assert_eq!(fbt_page.lower_bound_slot_id(&BTreeKey::str("c")), 2);
        assert_eq!(fbt_page.upper_bound_slot_id(&BTreeKey::str("c")), 3);
