...
pub struct SecBenchParams {
    ...
    #[clap(short, long, default_value = "20")]
    pub exec_time: usize,
    /// Update factor. The fraction of keys to update
    #[clap(short, long, default_value = "0.0")]
...
fn update_primary<M: MemPool>(
    params: &SecBenchParams,
    primary: &Arc<FosterBtree<M>>,
    perm: Permutation,
) {
    let update_count = (params.update_factor * params.num_keys as f64) as usize;
    for key in perm.into_iter().take(update_count) {
        let key_bytes = get_key_bytes(key, params.key_size);
        // Random value size between params.record_size 0.0 and 2.0
        let value_size_min = (params.record_size as f64 * 0.0) as usize;
        let value_size_max = (params.record_size as f64 * 2.0) as usize;
        let value_size = small_thread_rng().random_range(value_size_min..value_size_max);
        let value = get_new_value(value_size);
        primary.update(&key_bytes, &value).unwrap();
    }
}
