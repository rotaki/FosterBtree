use clap::Parser;
use fbtree::{bench_utils::*, bp::{BufferPool, LRUEvictionPolicy}, prelude::PagedHashMap, random::RandomKVs};
...
fn main() {
    ...
    let phm_unsafe_ps = gen_paged_hash_map_on_disk_with_unsafe_pointer_swizzling(bp_size, bucket_num);
    ...
    let (insert_duration_no_ps, get_duration_no_ps) =
        run_and_display_bench(insert_params.clone(), insert_kvs.clone(), get_params.clone(), get_kvs.clone(), phm_no_ps, "Pointer Swizzling OFF (disabled)");

    let (insert_duration_shared_ps, get_duration_shared_ps) =
        run_and_display_bench(insert_params.clone(), insert_kvs.clone(), get_params.clone(), get_kvs.clone(), phm_shared_ps.clone(), "Pointer Swizzling Shared by Thread with Atomic");

    let (insert_duration_unsafe_ps, get_duration_unsafe_ps) =
        run_and_display_bench(insert_params.clone(), insert_kvs.clone(), get_params.clone(), get_kvs.clone(), phm_unsafe_ps, "Pointer Swizzling Shared by Thread with Unsafe");

    let (insert_duration_local_ps, get_duration_local_ps) =
        run_and_display_bench(insert_params.clone(), insert_kvs.clone(), get_params.clone(), get_kvs.clone(), phm_local_ps, "Pointer Swizzling Local Thread Unsafe");
    ...
    println!("Duration Insert without Pointer Swizzling: {:?}", insert_duration_no_ps);
    println!("Duration Insert with Pointer Swizzling (Atomic Shared): {:?}", insert_duration_shared_ps);
    println!("Duration Insert with Pointer Swizzling (Unsafe Shared): {:?}", insert_duration_unsafe_ps);
    println!("Duration Insert with Pointer Swizzling (Thread Local Unsafe): {:?}\n", insert_duration_local_ps);

    println!("Duration Get without Pointer Swizzling: {:?}", get_duration_no_ps);
    println!("Duration Get with Pointer Swizzling (Atomic Shared): {:?}", get_duration_shared_ps);
    println!("Duration Get with Pointer Swizzling (Unsafe Shared): {:?}", get_duration_unsafe_ps);
    println!("Duration Get with Pointer Swizzling (Thread Local Unsafe): {:?}", get_duration_local_ps);
}