...
pub fn load_table(params: &Params, table: &Arc<impl NonUniqueKeyIndex + Send + Sync + 'static>) {
    (0..params.num_keys).for_each(|i| {
        ...
        table.append(&key, &value).unwrap();
    })
}

pub fn execute_workload(
    params: &Params,
    table: Arc<impl NonUniqueKeyIndex + Send + Sync + 'static>,
) -> Duration {
...
fn get_index<M: MemPool>(bp: Arc<M>, _params: &Params) -> Arc<HeapStore<M>> {
    Arc::new(HeapStore::new(ContainerKey::new(0, 0), bp))
}
...
fn main() {
    ...
    let num_frames = if params.bp_size == 0 {
        ...
        (net_size as f64 * 1.2 / PAGE_SIZE as f64) as usize
    } else {
